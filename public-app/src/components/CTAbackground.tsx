...
function shouldInitWebGL(): boolean {
 ...
 *
 * All props are optional. See LiquidEtherProps below for full list.
 */

const defaultColors = ["#000000", "#A40107", "#C70515"];
...
type SimProps = Record<string, unknown>;
...
`;
    ...
    class ShaderPass {
      ...
      constructor(props: ShaderPassBag) {
        this.props = props ?? {};
    ...
    class Advection extends ShaderPass {
      ...
      constructor(simProps: SimProps) {
        const cellScale = simProps["cellScale"] as THREE.Vector2;
        const fboSize = simProps["fboSize"] as THREE.Vector2;
        const dt = simProps["dt"] as number;
        const src = simProps["src"] as THREE.WebGLRenderTarget;
        const dst = simProps["dst"] as THREE.WebGLRenderTarget;
        super({
          material: {
    ...
    class ExternalForce extends ShaderPass {
      ...
      constructor(simProps: SimProps) {
        const dst = simProps["dst"] as THREE.WebGLRenderTarget;
        super({ output: dst });
        this.setupMouse(simProps);
      }
      setupMouse(simProps: SimProps): void {
        const cellScale = simProps["cellScale"] as THREE.Vector2;
        const cursorSize = simProps["cursor_size"] as number;
        super.init();
    ...
    class Viscous extends ShaderPass {
      constructor(simProps: SimProps) {
        const boundarySpace = simProps["boundarySpace"] as THREE.Vector2;
        const src = simProps["src"] as THREE.WebGLRenderTarget;
        const dst_ = simProps["dst_"] as THREE.WebGLRenderTarget;
        const viscous = simProps["viscous"] as number;
        const cellScale = simProps["cellScale"] as THREE.Vector2;
        const dt = simProps["dt"] as number;
        const dst = simProps["dst"] as THREE.WebGLRenderTarget;
        super({
      ...
      update(...args: unknown[]): unknown {
        ...
        const out0 = this.props.output0;
        const out1 = this.props.output1;
        if (!out0 || !out1) return undefined;
        ...
        for (let i = 0; i < iter; i++) {
          if (i % 2 === 0) {
            fbo_in = out0 as THREE.WebGLRenderTarget;
            fbo_out = out1 as THREE.WebGLRenderTarget;
          } else {
            fbo_in = out1 as THREE.WebGLRenderTarget;
            fbo_out = out0 as THREE.WebGLRenderTarget;
          }
      ...
      update(...args: unknown[]) {
        ...
        if (this.uniforms && vel) {
          this.uniforms.velocity.value = (
            vel as THREE.WebGLRenderTarget
          ).texture;
        }
        super.update();
      }
    ...
    class Poisson extends ShaderPass {
      ...
      update(...args: unknown[]): unknown {
        ...
        for (let i = 0; i < iter; i++) {
          if (i % 2 === 0) {
            p_in = out0 as THREE.WebGLRenderTarget;
            p_out = out1 as THREE.WebGLRenderTarget;
          } else {
            p_in = out1 as THREE.WebGLRenderTarget;
            p_out = out0 as THREE.WebGLRenderTarget;
          }
    ...
    class Pressure extends ShaderPass {
      constructor(simProps: SimProps) {
        const boundarySpace = simProps["boundarySpace"] as THREE.Vector2;
        const src_p = simProps["src_p"] as THREE.WebGLRenderTarget;
        const src_v = simProps["src_v"] as THREE.WebGLRenderTarget;
        const cellScale = simProps["cellScale"] as THREE.Vector2;
        const dt = simProps["dt"] as number;
        const dst = simProps["dst"] as THREE.WebGLRenderTarget;
        super({
          material: {
            ...
            uniforms: {
              boundarySpace: { value: boundarySpace },
              pressure: { value: src_p.texture },
              velocity: { value: src_v.texture },
              px: { value: cellScale },
              dt: { value: dt }
            }
          },
      ...
      update(...args: unknown[]) {
        ...
        if (this.uniforms && vel && pressure) {
          ...
          this.uniforms.pressure.value = (
            pressure as THREE.WebGLRenderTarget
          ).texture;
        }
        super.update();
      }
    ...
    class Simulation {
      ...
      resize() {
        this.calcSize();
        for (const key in this.fbos) {
          const rt = this.fbos[key] as THREE.WebGLRenderTarget;
          rt.setSize(this.fboSize.x, this.fboSize.y);
        }
      }
      update() {
        ...
        let vel: unknown = this.fbos["vel_1"];
        ...
        this.output = new THREE.Mesh(
          ...
          new THREE.RawShaderMaterial({
            ...
            uniforms: {
              velocity: {
                value: (
                  this.simulation.fbos["vel_0"] as THREE.WebGLRenderTarget
                ).texture
              },
    ...
    class WebGLManager implements LiquidEtherWebGL {
      ...
      constructor(props: SimProps) {
        this.props = props;
        const $wrapper = props["$wrapper"] as HTMLElement;
        Common.init($wrapper);
        Mouse.init($wrapper);
    ...
    try {
      ...
      const roOptions = { box: "content-box" as ResizeObserverBoxOptions };
      ro.observe(container, roOptions);
      resizeObserverRef.current = ro;
    } catch (error) {
      console.warn(
        "[CTAbackground] WebGL init failed; using CSS background only.",
        error
      );
