...
export default function AccountDetailsPage() {
  ...
  const [nonMfaRecommendedAccounts, setNonMfaRecommendedAccounts] = useState<
    AccountEntity[]
  >([]);
  ...
  useEffect(() => {
    ...
    fetchAccountByCode(code)
      .then((res) => {
        ...
        if (wantsDailyDrop && dropPriceListId && res?.priceTier?.priceList) {
  ...
  const onSubmit = async () => {
    ...
    if (
      ...
      (account.availabilityStatus === "IN_USE" ||
        account.availabilityStatus === "NOT_AVAILABLE")
    ) {
      return;
    }
    ...
    <main className="min-h-screen bg-black text-white">
      {showUnavailableModal && (
        <div className="fixed inset-0 z-[999] flex items-center justify-center bg-black/80">
          <div className="w-[90%] max-w-2xl border border-red-500 rounded-xl p-10 text-center bg-black">
            <h2 className="text-2xl sm:text-4xl font-bold text-white mb-4">
              ACCOUNT WILL BE AVAILABLE IN
            </h2>

            <p className="text-xl text-neutral-300 mb-8">
              <CountdownTimer targetDate={String(account.currentExpireAt)} />
            </p>
      ...
      <div className="pt-[110px] px-4 lg:px-10">
          ...
          <div className="col-span-12 lg:col-span-5 space-y-6 mb-4">
            ...
            <div className="grid grid-cols-[auto_1fr_auto] grid-rows-2 gap-x-4 gap-y-1 items-center">
              ...
              <div className="row-span-2">
                <Image
                  ...
                  width={56}
                  height={56}
                  className="object-contain w-12 h-12 tablet:w-20 tablet:h-20 "
                  priority
                />
              </div>
              ...
              <div className="row-span-2 flex items-center justify-end">
                <button
                  type="button"
                  className="flex items-center gap-1 text-[#D5D5D5] text-xs tablet:text-sm cursor-pointer hover:text-blue-300"
                  onClick={() =>
                    window.open(
                ...
                >
                  Account Info
                  <span className="flex items-center justify-center w-4 h-4 rounded-full bg-[#D5D5D5] text-black text-[10px] font-bold no-underline">
                    <IoPersonSharp className="text-white w-3 h-3 fill-white" />
                  </span>
                </button>
              </div>
              ...
              <div className="flex items-center gap-3">
                <div
                  className={cn(
                    "bg-[#F1F1F1] text-[#616161] px-2 py-0.5 text-xs rounded-sm",
                    staatliches.className
                  )}
                >
                  {account.isCompetitive
                    ? `COMPE - ${account.priceTier.code}`
                    : `UNRATED - ${account.priceTier.code}`}
                </div>
              </div>
            </div>
            ...
            <div className="space-y-2 text-sm text-[#F9FAFB]">
              <div className="flex justify-between items-center">
                ...
                {account.availabilityStatus == "IN_USE" && (
                  <div className="flex items-center gap-2 font-bold">
                    <span className="w-2 h-2 bg-[#C70515] rounded-full" />
                    Unavailable - On Rent
                  </div>
                )}
              </div>
              <div className="flex justify-between font-semibold">
                ...
                <p className="font-semibold">
                  {formatRentDuration(account.totalRentHour)}
                </p>
              </div>
              <div className="flex justify-between">
                <p>Region: </p>
                <p className="font-semibold">IDN</p>
              </div>
              <div className="flex justify-between">
                <p>Rank: </p>
            ...
            {/* SKIN LIST */}
            <div className="space-y-3 bg-[#1C1C1C] p-3 text-sm text-[#F9FAFB] rounded-lg">
              <div className="flex justify-between">
                <span>
                  <div
                  ...
                  >
                    ...
                    >
                      <IoCaretDown className="w-3 h-3 text-white" />
                    </div>
                  </div>
                </span>

                <p className="font-semibold">{account.skinList.length} Skins</p>
              </div>
              {showSkins && (
                <div className="space-y-3">
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full rounded-md bg-neutral-800 border border-neutral-700 pl-9 pr-3 py-2 text-sm text-white placeholder:text-white focus:outline-none focus:border-red-500"
                    />
                  </div>

                  {/* LIST */}
                  <div
                    className="flex flex-wrap gap-2 max-h-[240px] overflow-y-auto pr-1 [&::-webkit-scrollbar]:w-2 
                  ...
                  >
                    {filteredSkins.length === 0 ? (
                      <p className="text-sm text-white">No skins found</p>
                    ) : (
            ...
            {/* RENT / BOOK SECTION */}
            <div className="bg-neutral-900 rounded-sm xl:p-4 p-2 space-y-4">
              {/* MODE TOGGLE */}
              <div className="grid grid-cols-2 gap-0">
                <button
                  type="button"
                  disabled={rentForbidden}
                  onClick={() => !rentForbidden && setMode("RENT")}
            ...
            <button
              onClick={onSubmit}
              disabled={
                isDisabled || submitting || (mode === "RENT" && rentForbidden)
              }
              className={`w-full font-semibold py-3 rounded-md transition
                  ${
                    isDisabled ||
                    submitting ||
                    (mode === "RENT" && rentForbidden)
                      ? "bg-neutral-700 text-neutral-400 cursor-not-allowed"
            ...
            {/* FOOTER */}
            <div className="text-center text-xs text-neutral-400 space-y-3">
              ...
              <button
                type="button"
                className="w-full border-[0.5px] border-gray-700 hover:bg-neutral-600 py-3 rounded-lg text-white"
                onClick={() =>
                  window.open(
        ...
        <div className="max-w-[1920px] mx-auto grid grid-cols-12 gap-8 desktop:hidden">
          ...
          <div className="col-span-12 space-y-6 mb-4">
            <div className="grid tablet:grid-cols-2 grid-cols-1 tablet:gap-2 gap-5">
              <div className="flex flex-col">
                ...
                <div className="grid grid-cols-[auto_1fr_auto] grid-rows-2 gap-x-4 gap-y-1 items-center">
                  ...
                  {/* COL 2 ROW 1 — TITLE */}
                  <h1 className="tablet:text-3xl text-xl font-bold text-white tracking-wide leading-tight">
                    <span className="uppercase">{account.accountRank}</span>
                    <span className="text-neutral-400"> | </span>
                    <span>{account.accountCode}</span>
                  </h1>
                  ...
                  {/* COL 2 ROW 2 — PRICE TIER + STATUS */}
                  <div className="flex items-center tablet:items-start gap-3">
                    <div
                      className={cn(
                        "bg-[#F1F1F1] text-[#616161] px-2 py-0.5 text-xs rounded-sm",
                        staatliches.className
                ...
                {/* META */}
                <div className="space-y-2 text-xs tablet:text-sm text-[#F9FAFB] max-tablet:pt-8">
                  <div className="flex justify-between items-center">
                    <span>Status</span>
                    {account.availabilityStatus == "AVAILABLE" && (
                  ...
                  <div className="flex justify-between">
                    <p>Rank: </p>
                    <p className="">{account.accountRank}</p>
                  </div>
                </div>
              </div>
              {/* SKIN LIST */}
              <div className="space-y-3 bg-[#1C1C1C] px-4 tablet:pt-5 pt-3 text-xs text-[#F9FAFB] rounded-xl">
                <div className="flex justify-between">
                  <span>
                    <div
                      className="flex items-center justify-between cursor-pointer select-none"
                    ...
                    >
                      ...
                      >
                        <IoCaretDown className="w-2 h-2 text-white" />
                      </div>
                    </div>
                  </span>

                  <p className="font-semibold">
                    {account.skinList.length} Skins
                  </p>
                </div>
                {showSkins && (
                  <div className="space-y-3">
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        value={skinSearch}
                        onChange={(e) => setSkinSearch(e.target.value)}
                        className="w-full rounded-md bg-neutral-800 border border-neutral-700 pl-9 pr-3 py-2 text-xs text-white placeholder:text-white focus:outline-none focus:border-red-500"
                      />
                    </div>

                    {/* LIST */}
                    <div
                      className="flex flex-wrap gap-2 pb-4 max-h-[150px] tablet:max-h-[180px] overflow-y-auto pr-1 [&::-webkit-scrollbar]:w-2 
                                        [&::-webkit-scrollbar-track]:bg-neutral-800
                                        [&::-webkit-scrollbar-track]:rounded-full
                                        [&::-webkit-scrollbar-thumb]:bg-neutral-200
                                        [&::-webkit-scrollbar]:rounded-full
                    ...
                    >
                      {filteredSkins.length === 0 ? (
                        <p className="text-xs text-white">No skins found</p>
                      ) : (
                        filteredSkins.map((skin, i) => (
                          <Badge
                            key={skin.id ?? i}
            ...
            <div className="bg-neutral-900 rounded-sm p-4 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-0">
                <button
                  ...
                  disabled={rentForbidden}
                  onClick={() => !rentForbidden && setMode("RENT")}
                  className={`sm:text-sm text-xs font-semibold py-2 rounded-md transition max-tablet:min-h-[80px] flex items-center justify-center
                    ${
                      rentForbidden
                        ? mode === "RENT"
                          ? "bg-neutral-700 text-neutral-400 cursor-not-allowed"
            ...
            <div className="text-center text-xs text-neutral-400 space-y-3">
              ...
              <button
                type="button"
                className="w-full border-[0.5px] border-gray-700 hover:bg-neutral-600 tablet:py-3 py-5 rounded-lg text-white"
                onClick={() =>
                  window.open(
                    "https://wa.me/6285175343447?text=Halo%20admin%20VALSEWA%20aku%20butuh%20bantuan%20dong",
                    "_blank"
